...
function pagoVacaciones(sueldo) {
  let pagoVacas = (+sueldoBasico(sueldo) / 22) * 1.1 * 1.25 * diasVacaciones();
...
const liquidacion = () => {
  ...
  const radioAguinaldo = document.querySelector(
    'input[name="aguinaldo"]:checked'
  ).value;
  ...
  const radioVacaciones = document.querySelector(
    'input[name="vacaciones"]:checked'
  ).value;
  ...
  const remuneracionVacacionesTabla = document.querySelector(
    ".remuneracionVacacionesTabla"
  );

  aguinaldoTabla.textContent = `$${aguinaldo(sueldo).toFixed(2)}`;
  vacacionesTabla.textContent = `${diasVacaciones()}`;
  remuneracionVacacionesTabla.textContent = `$${pagoVacaciones(sueldo).toFixed(
    2
  )}`;
...
function datos() {
  ...
  if (
    ...
    sueldo === "" ||
    legajo === "" ||
    isNaN(ingreso.getDate())
    ) {
    alert("Faltó completar campos");
    cont.classList.add("d-none");
  } else {
    console.log(ingreso.getDate())
    liquidacion();
  }
}
